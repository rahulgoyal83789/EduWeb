...
void main() {
    ...
    // Multiplier was 2.0, which forces vignette to 0 at a distance of 0.5 from
    // centre. That is the circle inscribed in the viewport, so the four corners
    // were always fully black no matter what vignetteStrength was set to. 1.25
    // moves the zero point out to 0.8, past the 0.707 corner distance, so the
    // grid now reaches into all four corners.
    // (No backticks in this comment: it lives inside a JS template literal.)
    float vignette = 1.0 - pow(vignetteDistance * 1.25, vignetteStrength);
    ...
    // Listeners live on window, not on the container. This grid is used as a
    // full page backdrop with pointer-events:none, so the container itself never
    // receives a single mouse event and the interaction silently did nothing.
